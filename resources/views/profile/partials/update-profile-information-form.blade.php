<section>
  ...
  <form action="{{ route('profile.update', $user) }}" class="mt-6 space-y-6" method="post">
    @csrf
    @method('patch')

    <div>
      <x-input-label :value="__('Name')" for="name" />
      <x-text-input :value="old('name', $user->name)" autocomplete="name" autofocus class="mt-1 block w-full" id="name" name="name"
        required type="text" />
